/* Process model C form file: gsrc_mobil.pr.c */
/* Portions of this file copyright 1986-2008 by OPNET Technologies, Inc. */



/* This variable carries the header into the object file */
const char gsrc_mobil_pr_c [] = "MIL_3_Tfile_Hdr_ 145A 30A modeler 7 518A80C8 518A80C8 1 wpd FeiTong 0 0 none none 0 0 none 0 0 0 0 0 0 0 0 1e80 8                                                                                                                                                                                                                                                                                                                                                                                                              ";
#include <string.h>
...
#define FIN_PREAMBLE_DEC	gsrc_mobil_state *op_sv_ptr;
#define FIN_PREAMBLE_CODE	\
		op_sv_ptr = ((gsrc_mobil_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr));
...
extern "C" {
#endif
	void gsrc_mobil (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Obtype _op_gsrc_mobil_init (int * init_block_ptr);
	void _op_gsrc_mobil_diag (OP_SIM_CONTEXT_ARG_OPT);
	void _op_gsrc_mobil_terminate (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Address _op_gsrc_mobil_alloc (VosT_Obtype, int);
	void _op_gsrc_mobil_svar (void *, const char *, void **);
...
void
gsrc_mobil (OP_SIM_CONTEXT_ARG_OPT)
	{
#if !defined (VOSD_NO_FIN)
	...
	FIN_MT (gsrc_mobil ());
		...
		FSM_ENTER ("gsrc_mobil")

		FSM_BLOCK_SWITCH
			{
			/*---------------------------------------------------------*/
			/** state (init) enter executives **/
			FSM_STATE_ENTER_FORCED_NOLABEL (0, "init", "gsrc_mobil [init enter execs]")
				FSM_PROFILE_SECTION_IN ("gsrc_mobil [init enter execs]", state0_enter_exec)
				{
				// Obtain related ID
			...
			FSM_STATE_EXIT_FORCED (0, "init", "gsrc_mobil [init exit execs]")


			/** state (init) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "init", "idle", "tr_0", "gsrc_mobil [init -> idle : default / ]")
				/*---------------------------------------------------------*/



			/** state (idle) enter executives **/
			FSM_STATE_ENTER_UNFORCED (1, "idle", state1_enter_exec, "gsrc_mobil [idle enter execs]")

			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (3,"gsrc_mobil")


			/** state (idle) exit executives **/
			FSM_STATE_EXIT_UNFORCED (1, "idle", "gsrc_mobil [idle exit execs]")


			/** state (idle) transition processing **/
			FSM_PROFILE_SECTION_IN ("gsrc_mobil [idle trans conditions]", state1_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 2, state2_enter_exec, ;, "SEND_DATA_S", "", "idle", "send DATA S", "tr_3", "gsrc_mobil [idle -> send DATA S : SEND_DATA_S / ]")
				FSM_CASE_TRANSIT (1, 3, state3_enter_exec, ;, "SEND_DATA_R", "", "idle", "send DATA R", "tr_4", "gsrc_mobil [idle -> send DATA R : SEND_DATA_R / ]")
				FSM_CASE_TRANSIT (2, 4, state4_enter_exec, ;, "END", "", "idle", "end", "tr_6", "gsrc_mobil [idle -> end : END / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_FORCED (2, "send DATA S", state2_enter_exec, "gsrc_mobil [send DATA S enter execs]")
				FSM_PROFILE_SECTION_IN ("gsrc_mobil [send DATA S enter execs]", state2_enter_exec)
				{
			...
			FSM_STATE_EXIT_FORCED (2, "send DATA S", "gsrc_mobil [send DATA S exit execs]")


			/** state (send DATA S) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "send DATA S", "idle", "tr_2", "gsrc_mobil [send DATA S -> idle : default / ]")
				/*---------------------------------------------------------*/



			/** state (send DATA R) enter executives **/
			FSM_STATE_ENTER_FORCED (3, "send DATA R", state3_enter_exec, "gsrc_mobil [send DATA R enter execs]")
				FSM_PROFILE_SECTION_IN ("gsrc_mobil [send DATA R enter execs]", state3_enter_exec)
				{
				create_and_send_DATA();
				pk_num++;
				self_pk_num++;
			...
			FSM_STATE_EXIT_FORCED (3, "send DATA R", "gsrc_mobil [send DATA R exit execs]")


			/** state (send DATA R) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "send DATA R", "idle", "tr_5", "gsrc_mobil [send DATA R -> idle : default / ]")
				/*---------------------------------------------------------*/



			/** state (end) enter executives **/
			FSM_STATE_ENTER_UNFORCED (4, "end", state4_enter_exec, "gsrc_mobil [end enter execs]")
				FSM_PROFILE_SECTION_IN ("gsrc_mobil [end enter execs]", state4_enter_exec)
				{
				if(send_DATA)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (9,"gsrc_mobil")


			/** state (end) exit executives **/
			FSM_STATE_EXIT_UNFORCED (4, "end", "gsrc_mobil [end exit execs]")
...
void
_op_gsrc_mobil_diag (OP_SIM_CONTEXT_ARG_OPT)
	{
	/* No Diagnostic Block */
	}
...
void
_op_gsrc_mobil_terminate (OP_SIM_CONTEXT_ARG_OPT)
	{

	FIN_MT (_op_gsrc_mobil_terminate ())
...
/* local variable prs_ptr in _op_gsrc_mobil_svar function. */
#undef process_id
#undef node_id
#undef start_time
#undef interval
...
VosT_Obtype
_op_gsrc_mobil_init (int * init_block_ptr)
	{
	VosT_Obtype obtype = OPC_NIL;
	FIN_MT (_op_gsrc_mobil_init (init_block_ptr))

	obtype = Vos_Define_Object_Prstate ("proc state vars (gsrc_mobil)",
		sizeof (gsrc_mobil_state));
	*init_block_ptr = 0;
...
VosT_Address
_op_gsrc_mobil_alloc (VosT_Obtype obtype, int init_block)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	gsrc_mobil_state * ptr;
	FIN_MT (_op_gsrc_mobil_alloc (obtype))

	ptr = (gsrc_mobil_state *)Vos_Alloc_Object (obtype);
	if (ptr != OPC_NIL)
		{
		ptr->_op_current_block = init_block;
#if defined (OPD_ALLOW_ODB)
		ptr->_op_current_state = "gsrc_mobil [init enter execs]";
#endif
		}
...
void
_op_gsrc_mobil_svar (void * gen_ptr, const char * var_name, void ** var_p_ptr)
	{
	gsrc_mobil_state		*prs_ptr;

	FIN_MT (_op_gsrc_mobil_svar (gen_ptr, var_name, var_p_ptr))
	...
	prs_ptr = (gsrc_mobil_state *)gen_ptr;
