/* Process model C form file: gmac_multi_data.pr.c */
/* Portions of this file copyright 1986-2008 by OPNET Technologies, Inc. */



/* This variable carries the header into the object file */
const char gmac_multi_data_pr_c [] = "MIL_3_Tfile_Hdr_ 145A 30A modeler 7 4C29DD0D 4C29DD0D 1 china-0f9728557 Administrator 0 0 none none 0 0 none 0 0 0 0 0 0 0 0 1e80 8                                                                                                                                                                                                                                                                                                                                                                                            ";
#include <string.h>
...
typedef struct
	...
	double	                 		all_rx_time                                     ;
	double	                 		all_tx_time                                     ;
	int	                    		maximum_data_num                                ;
	int	                    		pk_num_in_RTS                                   ;
	int	                    		received_data_num                               ;
	} gmac_multi_data_state;
...
#define maximum_data_num        		op_sv_ptr->maximum_data_num
#define pk_num_in_RTS           		op_sv_ptr->pk_num_in_RTS
#define received_data_num       		op_sv_ptr->received_data_num
...
#define FIN_PREAMBLE_DEC	gmac_multi_data_state *op_sv_ptr;
#define FIN_PREAMBLE_CODE	\
		op_sv_ptr = ((gmac_multi_data_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr));
...
extern "C" {
#endif
	void gmac_multi_data (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Obtype _op_gmac_multi_data_init (int * init_block_ptr);
	void _op_gmac_multi_data_diag (OP_SIM_CONTEXT_ARG_OPT);
	void _op_gmac_multi_data_terminate (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Address _op_gmac_multi_data_alloc (VosT_Obtype, int);
	void _op_gmac_multi_data_svar (void *, const char *, void **);
...
void
gmac_multi_data (OP_SIM_CONTEXT_ARG_OPT)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	FIN_MT (gmac_multi_data ());
		...
		FSM_ENTER ("gmac_multi_data")

		FSM_BLOCK_SWITCH
			{
			/*---------------------------------------------------------*/
			/** state (init) enter executives **/
			FSM_STATE_ENTER_FORCED_NOLABEL (0, "init", "gmac_multi_data [init enter execs]")
				FSM_PROFILE_SECTION_IN ("gmac_multi_data [init enter execs]", state0_enter_exec)
				{
				// Obtain related ID
				...
				maximum_data_num = floor(((sleep_factor - 2) * durActive_half-durACK-SIFS)/(durDATA+SIFS))+1;//floor为向下取整函数
			...
			FSM_STATE_EXIT_FORCED (0, "init", "gmac_multi_data [init exit execs]")
				FSM_PROFILE_SECTION_IN ("gmac_multi_data [init exit execs]", state0_exit_exec)
				{
				/*
			...
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "init", "idle", "tr_1", "gmac_multi_data [init -> idle : default / ]")
				/*---------------------------------------------------------*/



			/** state (idle) enter executives **/
			FSM_STATE_ENTER_UNFORCED (1, "idle", state1_enter_exec, "gmac_multi_data [idle enter execs]")
				FSM_PROFILE_SECTION_IN ("gmac_multi_data [idle enter execs]", state1_enter_exec)
				{
				if(END)
				{
					op_ima_obj_attr_get(node_id,"Grade Record File",temp_file_name);
			...
			FSM_EXIT (3,"gmac_multi_data")


			/** state (idle) exit executives **/
			FSM_STATE_EXIT_UNFORCED (1, "idle", "gmac_multi_data [idle exit execs]")


			/** state (idle) transition processing **/
			FSM_PROFILE_SECTION_IN ("gmac_multi_data [idle trans conditions]", state1_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 2, state2_enter_exec, ;, "FROM_LOWER && transceiver_state", "", "idle", "GM proc", "tr_5", "gmac_multi_data [idle -> GM proc : FROM_LOWER && transceiver_state / ]")
				FSM_CASE_TRANSIT (1, 3, state3_enter_exec, ;, "SEND_GM", "", "idle", "send gm", "tr_7", "gmac_multi_data [idle -> send gm : SEND_GM / ]")
				FSM_CASE_TRANSIT (2, 6, state6_enter_exec, ;, "CONTEND_TO_SEND_RTS", "", "idle", "second contention", "tr_28", "gmac_multi_data [idle -> second contention : CONTEND_TO_SEND_RTS / ]")
				FSM_CASE_TRANSIT (3, 4, state4_enter_exec, ;, "GO_TO_SLEEP", "", "idle", "sleeping", "tr_32", "gmac_multi_data [idle -> sleeping : GO_TO_SLEEP / ]")
				FSM_CASE_TRANSIT (4, 5, state5_enter_exec, ;, "WAKING_TO_WAIT_RTS", "", "idle", "waking", "tr_34", "gmac_multi_data [idle -> waking : WAKING_TO_WAIT_RTS / ]")
				FSM_CASE_TRANSIT (5, 1, state1_enter_exec, ;, "default", "", "idle", "idle", "tr_88", "gmac_multi_data [idle -> idle : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_FORCED (2, "GM proc", state2_enter_exec, "gmac_multi_data [GM proc enter execs]")
				FSM_PROFILE_SECTION_IN ("gmac_multi_data [GM proc enter execs]", state2_enter_exec)
				{
			...
			FSM_STATE_EXIT_FORCED (2, "GM proc", "gmac_multi_data [GM proc exit execs]")


			/** state (GM proc) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "GM proc", "idle", "tr_6", "gmac_multi_data [GM proc -> idle : default / ]")
				/*---------------------------------------------------------*/



			/** state (send gm) enter executives **/
			FSM_STATE_ENTER_FORCED (3, "send gm", state3_enter_exec, "gmac_multi_data [send gm enter execs]")
				FSM_PROFILE_SECTION_IN ("gmac_multi_data [send gm enter execs]", state3_enter_exec)
				{
				if(op_subq_empty(grade_queue) == OPC_FALSE)
				{
			...
			FSM_STATE_EXIT_FORCED (3, "send gm", "gmac_multi_data [send gm exit execs]")


			/** state (send gm) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "send gm", "idle", "tr_87", "gmac_multi_data [send gm -> idle : default / ]")
				/*---------------------------------------------------------*/



			/** state (sleeping) enter executives **/
			FSM_STATE_ENTER_UNFORCED (4, "sleeping", state4_enter_exec, "gmac_multi_data [sleeping enter execs]")
				FSM_PROFILE_SECTION_IN ("gmac_multi_data [sleeping enter execs]", state4_enter_exec)
				{
				...
				if(op_intrpt_code() == SLEEP_FROM_FIRST_CODE)//sleep from first period
				{
					...
					}else if(pk_num_in_RTS>1 && pk_num_in_RTS==received_data_num)//有多个包，且全部收到
					{
						time = waking_time + durActive_half + 2*(durACK+SIFS)+(pk_num_in_RTS-1)*(durDATA+SIFS)-op_sim_time();
						//when wake up, go to "second contention" state
						op_intrpt_schedule_self(op_sim_time() + time, WAKING_TO_SEND_RTS_CODE);
					}
					printf("Node:%d, Grade:%d, time:%f,in sleeping period.\n\
							To sleeping from the first period.\n\
							pk_num_in_RTS is %d, received_data_num is %d.\n\
							After %f seconds, go to second period.\n",\
							node_address,grade,op_sim_time(),pk_num_in_RTS,received_data_num,time);
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (9,"gmac_multi_data")


			/** state (sleeping) exit executives **/
			FSM_STATE_EXIT_UNFORCED (4, "sleeping", "gmac_multi_data [sleeping exit execs]")


			/** state (sleeping) transition processing **/
			FSM_PROFILE_SECTION_IN ("gmac_multi_data [sleeping trans conditions]", state4_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 5, state5_enter_exec, ;, "WAKING_UP", "", "sleeping", "waking", "tr_37", "gmac_multi_data [sleeping -> waking : WAKING_UP / ]")
				FSM_CASE_TRANSIT (1, 6, state6_enter_exec, ;, "WAKING_TO_SEND_RTS", "", "sleeping", "second contention", "tr_40", "gmac_multi_data [sleeping -> second contention : WAKING_TO_SEND_RTS / ]")
				FSM_CASE_TRANSIT (2, 4, state4_enter_exec, ;, "default", "", "sleeping", "sleeping", "tr_91", "gmac_multi_data [sleeping -> sleeping : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_FORCED (5, "waking", state5_enter_exec, "gmac_multi_data [waking enter execs]")
				FSM_PROFILE_SECTION_IN ("gmac_multi_data [waking enter execs]", state5_enter_exec)
				{
				...
				//pkts_num_TV=op_subq_stat (data_queue, OPC_QSTAT_PKSIZE);//统计数据队列中数据包的个数。
				}
				FSM_PROFILE_SECTION_OUT (state5_enter_exec)

			/** state (waking) exit executives **/
			FSM_STATE_EXIT_FORCED (5, "waking", "gmac_multi_data [waking exit execs]")


			/** state (waking) transition processing **/
			FSM_TRANSIT_FORCE (8, state8_enter_exec, ;, "default", "", "waking", "first", "tr_57", "gmac_multi_data [waking -> first : default / ]")
				/*---------------------------------------------------------*/



			/** state (second contention) enter executives **/
			FSM_STATE_ENTER_FORCED (6, "second contention", state6_enter_exec, "gmac_multi_data [second contention enter execs]")
				FSM_PROFILE_SECTION_IN ("gmac_multi_data [second contention enter execs]", state6_enter_exec)
				{
			...
			FSM_STATE_EXIT_FORCED (6, "second contention", "gmac_multi_data [second contention exit execs]")


			/** state (second contention) transition processing **/
			FSM_TRANSIT_FORCE (9, state9_enter_exec, ;, "default", "", "second contention", "second", "tr_59", "gmac_multi_data [second contention -> second : default / ]")
				/*---------------------------------------------------------*/



			/** state (wait CTS) enter executives **/
			FSM_STATE_ENTER_FORCED (7, "wait CTS", state7_enter_exec, "gmac_multi_data [wait CTS enter execs]")
				FSM_PROFILE_SECTION_IN ("gmac_multi_data [wait CTS enter execs]", state7_enter_exec)
				{
				//send RTS
				if(pk_TV!=NULL)
				{
			...
			FSM_STATE_EXIT_FORCED (7, "wait CTS", "gmac_multi_data [wait CTS exit execs]")


			/** state (wait CTS) transition processing **/
			FSM_TRANSIT_FORCE (9, state9_enter_exec, ;, "default", "", "wait CTS", "second", "tr_89", "gmac_multi_data [wait CTS -> second : default / ]")
				/*---------------------------------------------------------*/



			/** state (first) enter executives **/
			FSM_STATE_ENTER_UNFORCED (8, "first", state8_enter_exec, "gmac_multi_data [first enter execs]")
				FSM_PROFILE_SECTION_IN ("gmac_multi_data [first enter execs]", state8_enter_exec)
				{
				//Received DATA from upper layer
				if(FROM_UPPER)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (17,"gmac_multi_data")


			/** state (first) exit executives **/
			FSM_STATE_EXIT_UNFORCED (8, "first", "gmac_multi_data [first exit execs]")


			/** state (first) transition processing **/
			FSM_PROFILE_SECTION_IN ("gmac_multi_data [first trans conditions]", state8_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 4, state4_enter_exec, ;, "SLEEP_FROM_FIRST", "", "first", "sleeping", "tr_56", "gmac_multi_data [first -> sleeping : SLEEP_FROM_FIRST / ]")
				FSM_CASE_TRANSIT (1, 12, state12_enter_exec, ;, "FROM_LOWER && transceiver_state", "", "first", "RCV(1)", "tr_74", "gmac_multi_data [first -> RCV(1) : FROM_LOWER && transceiver_state / ]")
				FSM_CASE_TRANSIT (2, 13, state13_enter_exec, ;, "SEND_CTS", "", "first", "wait DATA", "tr_76", "gmac_multi_data [first -> wait DATA : SEND_CTS / ]")
				FSM_CASE_TRANSIT (3, 14, state14_enter_exec, ;, "SEND_ACK", "", "first", "send ACK", "tr_79", "gmac_multi_data [first -> send ACK : SEND_ACK / ]")
				FSM_CASE_TRANSIT (4, 15, state15_enter_exec, ;, "CHECK_CHANNEL", "", "first", "carrier sense(1)", "tr_93", "gmac_multi_data [first -> carrier sense(1) : CHECK_CHANNEL / ]")
				FSM_CASE_TRANSIT (5, 8, state8_enter_exec, ;, "default", "", "first", "first", "tr_90", "gmac_multi_data [first -> first : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_UNFORCED (9, "second", state9_enter_exec, "gmac_multi_data [second enter execs]")
				FSM_PROFILE_SECTION_IN ("gmac_multi_data [second enter execs]", state9_enter_exec)
				{
				//Received DATA from upper layer
				if(FROM_UPPER)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (19,"gmac_multi_data")


			/** state (second) exit executives **/
			FSM_STATE_EXIT_UNFORCED (9, "second", "gmac_multi_data [second exit execs]")


			/** state (second) transition processing **/
			FSM_PROFILE_SECTION_IN ("gmac_multi_data [second trans conditions]", state9_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 7, state7_enter_exec, ;, "SEND_RTS", "", "second", "wait CTS", "tr_60", "gmac_multi_data [second -> wait CTS : SEND_RTS / ]")
				FSM_CASE_TRANSIT (1, 4, state4_enter_exec, ;, "SLEEP_FROM_SECOND", "", "second", "sleeping", "tr_61", "gmac_multi_data [second -> sleeping : SLEEP_FROM_SECOND / ]")
				FSM_CASE_TRANSIT (2, 10, state10_enter_exec, ;, "FROM_LOWER && transceiver_state", "", "second", "RCV(2)", "tr_66", "gmac_multi_data [second -> RCV(2) : FROM_LOWER && transceiver_state / ]")
				FSM_CASE_TRANSIT (3, 11, state11_enter_exec, ;, "SEND_DATA", "", "second", "send data", "tr_72", "gmac_multi_data [second -> send data : SEND_DATA / ]")
				FSM_CASE_TRANSIT (4, 16, state16_enter_exec, ;, "CHECK_CHANNEL", "", "second", "carrier sense(2)", "tr_95", "gmac_multi_data [second -> carrier sense(2) : CHECK_CHANNEL / ]")
				FSM_CASE_TRANSIT (5, 9, state9_enter_exec, ;, "default", "", "second", "second", "tr_92", "gmac_multi_data [second -> second : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_FORCED (10, "RCV(2)", state10_enter_exec, "gmac_multi_data [RCV(2) enter execs]")
				FSM_PROFILE_SECTION_IN ("gmac_multi_data [RCV(2) enter execs]", state10_enter_exec)
				{
				if(pk_TV!=NULL)
				{
					op_pk_destroy(pk_TV);
			...
			FSM_STATE_EXIT_FORCED (10, "RCV(2)", "gmac_multi_data [RCV(2) exit execs]")


			/** state (RCV(2)) transition processing **/
			FSM_TRANSIT_FORCE (9, state9_enter_exec, ;, "default", "", "RCV(2)", "second", "tr_71", "gmac_multi_data [RCV(2) -> second : default / ]")
				/*---------------------------------------------------------*/



			/** state (send data) enter executives **/
			FSM_STATE_ENTER_FORCED (11, "send data", state11_enter_exec, "gmac_multi_data [send data enter execs]")
				FSM_PROFILE_SECTION_IN ("gmac_multi_data [send data enter execs]", state11_enter_exec)
				{
				//send DATA
				if(pk_TV!=NULL)
				{
			...
			FSM_STATE_EXIT_FORCED (11, "send data", "gmac_multi_data [send data exit execs]")


			/** state (send data) transition processing **/
			FSM_TRANSIT_FORCE (9, state9_enter_exec, ;, "default", "", "send data", "second", "tr_73", "gmac_multi_data [send data -> second : default / ]")
				/*---------------------------------------------------------*/



			/** state (RCV(1)) enter executives **/
			FSM_STATE_ENTER_FORCED (12, "RCV(1)", state12_enter_exec, "gmac_multi_data [RCV(1) enter execs]")
				FSM_PROFILE_SECTION_IN ("gmac_multi_data [RCV(1) enter execs]", state12_enter_exec)
				{
				if(pk_TV!=NULL)
				{
					op_pk_destroy(pk_TV);
				...
				if(pk_type == rts_pk)
				{
					...
					if(grade_TV == grade+1)
					{
						if(op_ev_valid(wait_RTS_event))//the first time to receive RTS
						{
							...
							op_pk_nfd_get(pk_TV,"Data Num",&pk_num_in_RTS);
				...
				if(pk_type == data_pk)
				{
					...
					if(dest_TV == node_address)
					{
						if(op_ev_valid(wait_DATA_event))
						{
							...
							if(pk_num_in_RTS == received_data_num)//只有一个包要收 或者 多个包都已经收到，回复ACK
							{	
							...
							else if(pk_num_in_RTS>1 && received_data_num==1)//RTS中显示有多个包要来，而刚收到一个包，需要在下一个Active中回复ACK
							{
								time = waking_time + durActive_half-op_sim_time();
								op_intrpt_schedule_self(op_sim_time()+time,SEND_ACK_CODE);
								printf("Node:%d, Grade:%d, time:%f, in first period.\n\
										The # of pkts need to be received is: %d.\n\
										Now successfully received one DATA.\n\
										Go to next active period to reply ACK.\n",\
										node_address,grade,op_sim_time(),pk_num_in_RTS);
							}else if(received_data_num > 1 && pk_num_in_RTS > received_data_num)//表示还有包要收
							{
							...
							{
								op_sim_end("Err: pk_num_in_RTS is not available.","","","");
							}
						}else
						{
							op_pk_destroy(pk_TV);
			...
			FSM_STATE_EXIT_FORCED (12, "RCV(1)", "gmac_multi_data [RCV(1) exit execs]")


			/** state (RCV(1)) transition processing **/
			FSM_TRANSIT_FORCE (8, state8_enter_exec, ;, "default", "", "RCV(1)", "first", "tr_75", "gmac_multi_data [RCV(1) -> first : default / ]")
				/*---------------------------------------------------------*/



			/** state (wait DATA) enter executives **/
			FSM_STATE_ENTER_FORCED (13, "wait DATA", state13_enter_exec, "gmac_multi_data [wait DATA enter execs]")
				FSM_PROFILE_SECTION_IN ("gmac_multi_data [wait DATA enter execs]", state13_enter_exec)
				{
				//send CTS
				if(pk_TV!=NULL)
				{
			...
			FSM_STATE_EXIT_FORCED (13, "wait DATA", "gmac_multi_data [wait DATA exit execs]")


			/** state (wait DATA) transition processing **/
			FSM_TRANSIT_FORCE (8, state8_enter_exec, ;, "default", "", "wait DATA", "first", "tr_77", "gmac_multi_data [wait DATA -> first : default / ]")
				/*---------------------------------------------------------*/



			/** state (send ACK) enter executives **/
			FSM_STATE_ENTER_FORCED (14, "send ACK", state14_enter_exec, "gmac_multi_data [send ACK enter execs]")
				FSM_PROFILE_SECTION_IN ("gmac_multi_data [send ACK enter execs]", state14_enter_exec)
				{
				//send ACK
				if(pk_TV!=NULL)
				{
				...
				op_pk_nfd_set(pk_TV,"Dest",next_hop_1);
				op_pk_nfd_set(pk_TV,"Grade",grade);
				op_pk_nfd_set(pk_TV,"Data Num",pk_num_in_RTS);
				op_pk_nfd_set(pk_TV,"Data No",received_data_num);
				...
				if(pk_num_in_RTS==1||pk_num_in_RTS==received_data_num)
				{	
					...
					printf("Node:%d, Grade:%d, time:%f, in first period.\n\
							...
							node_address,grade,op_sim_time());
				}else if(pk_num_in_RTS>1 && received_data_num==1)//有多个包要收,目前只收到1个,现在回复ACK通知下一级别节点
				{
					time = durACK+SIFS+durDATA;
					wait_DATA_event = op_intrpt_schedule_self(op_sim_time() + time,SLEEP_FROM_FIRST_CODE);
					printf("Node:%d, Grade:%d, time:%f, in first period.\n\
							The # of pkts need to be received is: %d.\n\
							Now successfully received one DATA.\n\
			...
			FSM_STATE_EXIT_FORCED (14, "send ACK", "gmac_multi_data [send ACK exit execs]")


			/** state (send ACK) transition processing **/
			FSM_TRANSIT_FORCE (8, state8_enter_exec, ;, "default", "", "send ACK", "first", "tr_80", "gmac_multi_data [send ACK -> first : default / ]")
				/*---------------------------------------------------------*/



			/** state (carrier sense(1)) enter executives **/
			FSM_STATE_ENTER_FORCED (15, "carrier sense(1)", state15_enter_exec, "gmac_multi_data [carrier sense(1) enter execs]")
				FSM_PROFILE_SECTION_IN ("gmac_multi_data [carrier sense(1) enter execs]", state15_enter_exec)
				{
			...
			FSM_STATE_EXIT_FORCED (15, "carrier sense(1)", "gmac_multi_data [carrier sense(1) exit execs]")


			/** state (carrier sense(1)) transition processing **/
			FSM_TRANSIT_FORCE (8, state8_enter_exec, ;, "default", "", "carrier sense(1)", "first", "tr_94", "gmac_multi_data [carrier sense(1) -> first : default / ]")
				/*---------------------------------------------------------*/



			/** state (carrier sense(2)) enter executives **/
			FSM_STATE_ENTER_FORCED (16, "carrier sense(2)", state16_enter_exec, "gmac_multi_data [carrier sense(2) enter execs]")
				FSM_PROFILE_SECTION_IN ("gmac_multi_data [carrier sense(2) enter execs]", state16_enter_exec)
				{
			...
			FSM_STATE_EXIT_FORCED (16, "carrier sense(2)", "gmac_multi_data [carrier sense(2) exit execs]")


			/** state (carrier sense(2)) transition processing **/
			FSM_TRANSIT_FORCE (9, state9_enter_exec, ;, "default", "", "carrier sense(2)", "second", "tr_96", "gmac_multi_data [carrier sense(2) -> second : default / ]")
				/*---------------------------------------------------------*/
...
void
_op_gmac_multi_data_diag (OP_SIM_CONTEXT_ARG_OPT)
	{
	/* No Diagnostic Block */
	}
...
void
_op_gmac_multi_data_terminate (OP_SIM_CONTEXT_ARG_OPT)
	{

	FIN_MT (_op_gmac_multi_data_terminate ())
...
/* local variable prs_ptr in _op_gmac_multi_data_svar function. */
#undef process_id
#undef node_id
#undef node_address
#undef grade
...
#undef all_rx_time
#undef all_tx_time
#undef maximum_data_num
#undef pk_num_in_RTS
#undef received_data_num
...
VosT_Obtype
_op_gmac_multi_data_init (int * init_block_ptr)
	{
	VosT_Obtype obtype = OPC_NIL;
	FIN_MT (_op_gmac_multi_data_init (init_block_ptr))

	obtype = Vos_Define_Object_Prstate ("proc state vars (gmac_multi_data)",
		sizeof (gmac_multi_data_state));
	*init_block_ptr = 0;
...
VosT_Address
_op_gmac_multi_data_alloc (VosT_Obtype obtype, int init_block)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	gmac_multi_data_state * ptr;
	FIN_MT (_op_gmac_multi_data_alloc (obtype))

	ptr = (gmac_multi_data_state *)Vos_Alloc_Object (obtype);
	if (ptr != OPC_NIL)
		{
		ptr->_op_current_block = init_block;
#if defined (OPD_ALLOW_ODB)
		ptr->_op_current_state = "gmac_multi_data [init enter execs]";
#endif
		}
...
void
_op_gmac_multi_data_svar (void * gen_ptr, const char * var_name, void ** var_p_ptr)
	{
	gmac_multi_data_state		*prs_ptr;

	FIN_MT (_op_gmac_multi_data_svar (gen_ptr, var_name, var_p_ptr))
	...
	prs_ptr = (gmac_multi_data_state *)gen_ptr;
	...
	if (strcmp ("maximum_data_num" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->maximum_data_num);
		FOUT
		}
	if (strcmp ("pk_num_in_RTS" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->pk_num_in_RTS);
		FOUT
		}
	if (strcmp ("received_data_num" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->received_data_num);
		FOUT
		}
	*var_p_ptr = (void *)OPC_NIL;
