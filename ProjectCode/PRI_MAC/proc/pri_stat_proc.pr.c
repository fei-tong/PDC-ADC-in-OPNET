/* Process model C form file: pri_stat_proc.pr.c */
/* Portions of this file copyright 1986-2008 by OPNET Technologies, Inc. */



/* This variable carries the header into the object file */
const char pri_stat_proc_pr_c [] = "MIL_3_Tfile_Hdr_ 145A 30A modeler 7 52F07680 52F07680 1 wpd FeiTong 0 0 none none 0 0 none 0 0 0 0 0 0 0 0 1e80 8                                                                                                                                                                                                                                                                                                                                                                                                              ";
#include <string.h>
...
#define FIN_PREAMBLE_DEC	pri_stat_proc_state *op_sv_ptr;
#define FIN_PREAMBLE_CODE	\
		op_sv_ptr = ((pri_stat_proc_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr));
...
//body
	...
	/* from state "end" in module "collect data" */
...
extern "C" {
#endif
	void pri_stat_proc (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Obtype _op_pri_stat_proc_init (int * init_block_ptr);
	void _op_pri_stat_proc_diag (OP_SIM_CONTEXT_ARG_OPT);
	void _op_pri_stat_proc_terminate (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Address _op_pri_stat_proc_alloc (VosT_Obtype, int);
	void _op_pri_stat_proc_svar (void *, const char *, void **);
...
void
pri_stat_proc (OP_SIM_CONTEXT_ARG_OPT)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	FIN_MT (pri_stat_proc ());

		{


		FSM_ENTER ("pri_stat_proc")

		FSM_BLOCK_SWITCH
			{
			/*---------------------------------------------------------*/
			/** state (idle) enter executives **/
			FSM_STATE_ENTER_UNFORCED (0, "idle", state0_enter_exec, "pri_stat_proc [idle enter execs]")

			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (1,"pri_stat_proc")


			/** state (idle) exit executives **/
			FSM_STATE_EXIT_UNFORCED (0, "idle", "pri_stat_proc [idle exit execs]")


			/** state (idle) transition processing **/
			FSM_PROFILE_SECTION_IN ("pri_stat_proc [idle trans conditions]", state0_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 1, state1_enter_exec, ;, "REMOTE_STAT", "", "idle", "stat", "tr_1", "pri_stat_proc [idle -> stat : REMOTE_STAT / ]")
				FSM_CASE_TRANSIT (1, 2, state2_enter_exec, ;, "END", "", "idle", "end", "tr_9", "pri_stat_proc [idle -> end : END / ]")
				FSM_CASE_TRANSIT (2, 0, state0_enter_exec, ;, "default", "", "idle", "idle", "tr_0", "pri_stat_proc [idle -> idle : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_FORCED (1, "stat", state1_enter_exec, "pri_stat_proc [stat enter execs]")
				FSM_PROFILE_SECTION_IN ("pri_stat_proc [stat enter execs]", state1_enter_exec)
				{
				deal_with_ici();
				}
			...
			FSM_STATE_EXIT_FORCED (1, "stat", "pri_stat_proc [stat exit execs]")


			/** state (stat) transition processing **/
			FSM_TRANSIT_FORCE (0, state0_enter_exec, ;, "default", "", "stat", "idle", "tr_2", "pri_stat_proc [stat -> idle : default / ]")
				/*---------------------------------------------------------*/



			/** state (end) enter executives **/
			FSM_STATE_ENTER_FORCED (2, "end", state2_enter_exec, "pri_stat_proc [end enter execs]")
				FSM_PROFILE_SECTION_IN ("pri_stat_proc [end enter execs]", state2_enter_exec)
				{
				func_sim_end();
				}
			...
			FSM_STATE_EXIT_FORCED (2, "end", "pri_stat_proc [end exit execs]")


			/** state (end) transition processing **/
			FSM_TRANSIT_FORCE (0, state0_enter_exec, ;, "default", "", "end", "idle", "tr_7", "pri_stat_proc [end -> idle : default / ]")
				/*---------------------------------------------------------*/



			/** state (init) enter executives **/
			FSM_STATE_ENTER_FORCED_NOLABEL (3, "init", "pri_stat_proc [init enter execs]")
				FSM_PROFILE_SECTION_IN ("pri_stat_proc [init enter execs]", state3_enter_exec)
				{
				// Obtain related ID
			...
			FSM_STATE_EXIT_FORCED (3, "init", "pri_stat_proc [init exit execs]")


			/** state (init) transition processing **/
			FSM_TRANSIT_FORCE (0, state0_enter_exec, ;, "default", "", "init", "idle", "tr_10", "pri_stat_proc [init -> idle : default / ]")
				/*---------------------------------------------------------*/
...
void
_op_pri_stat_proc_diag (OP_SIM_CONTEXT_ARG_OPT)
	{
	/* No Diagnostic Block */
	}
...
void
_op_pri_stat_proc_terminate (OP_SIM_CONTEXT_ARG_OPT)
	{

	FIN_MT (_op_pri_stat_proc_terminate ())
...
/* local variable prs_ptr in _op_pri_stat_proc_svar function. */
#undef node_id
#undef process_id
#undef node_type
...
VosT_Obtype
_op_pri_stat_proc_init (int * init_block_ptr)
	{
	VosT_Obtype obtype = OPC_NIL;
	FIN_MT (_op_pri_stat_proc_init (init_block_ptr))

	obtype = Vos_Define_Object_Prstate ("proc state vars (pri_stat_proc)",
		sizeof (pri_stat_proc_state));
	*init_block_ptr = 6;
...
VosT_Address
_op_pri_stat_proc_alloc (VosT_Obtype obtype, int init_block)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	pri_stat_proc_state * ptr;
	FIN_MT (_op_pri_stat_proc_alloc (obtype))

	ptr = (pri_stat_proc_state *)Vos_Alloc_Object (obtype);
	if (ptr != OPC_NIL)
		{
		ptr->_op_current_block = init_block;
#if defined (OPD_ALLOW_ODB)
		ptr->_op_current_state = "pri_stat_proc [init enter execs]";
#endif
		}
...
void
_op_pri_stat_proc_svar (void * gen_ptr, const char * var_name, void ** var_p_ptr)
	{
	pri_stat_proc_state		*prs_ptr;

	FIN_MT (_op_pri_stat_proc_svar (gen_ptr, var_name, var_p_ptr))
	...
	prs_ptr = (pri_stat_proc_state *)gen_ptr;
