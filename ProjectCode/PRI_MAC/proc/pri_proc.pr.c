/* Process model C form file: pri_proc.pr.c */
/* Portions of this file copyright 1986-2008 by OPNET Technologies, Inc. */



/* This variable carries the header into the object file */
const char pri_proc_pr_c [] = "MIL_3_Tfile_Hdr_ 145A 30A modeler 7 52F3CBC1 52F3CBC1 1 wpd FeiTong 0 0 none none 0 0 none 0 0 0 0 0 0 0 0 1e80 8                                                                                                                                                                                                                                                                                                                                                                                                              ";
#include <string.h>
...
#define FIN_PREAMBLE_DEC	pri_proc_state *op_sv_ptr;
#define FIN_PREAMBLE_CODE	\
		op_sv_ptr = ((pri_proc_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr));
...
//body
	...
	for(list_i=0;list_i<list_size;list_i++){
		list_element = op_prg_list_access(queue_stat_list,list_i);
		op_stat_write_t(QStatus_handle,*list_element,list_i);
...
extern "C" {
#endif
	void pri_proc (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Obtype _op_pri_proc_init (int * init_block_ptr);
	void _op_pri_proc_diag (OP_SIM_CONTEXT_ARG_OPT);
	void _op_pri_proc_terminate (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Address _op_pri_proc_alloc (VosT_Obtype, int);
	void _op_pri_proc_svar (void *, const char *, void **);
...
void
pri_proc (OP_SIM_CONTEXT_ARG_OPT)
	{
#if !defined (VOSD_NO_FIN)
	...
	FIN_MT (pri_proc ());
		...
		FSM_BLOCK_SWITCH
			...
			FSM_STATE_ENTER_FORCED_NOLABEL (0, "init", "pri_proc [init enter execs]")
				FSM_PROFILE_SECTION_IN ("pri_proc [init enter execs]", state0_enter_exec)
				{
				// Obtain related ID
			...
			FSM_STATE_EXIT_FORCED (0, "init", "pri_proc [init exit execs]")
				FSM_PROFILE_SECTION_IN ("pri_proc [init exit execs]", state0_exit_exec)
				{
				/*
			...
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "init", "idle", "tr_1", "pri_proc [init -> idle : default / ]")
				/*---------------------------------------------------------*/



			/** state (idle) enter executives **/
			FSM_STATE_ENTER_UNFORCED (1, "idle", state1_enter_exec, "pri_proc [idle enter execs]")
				FSM_PROFILE_SECTION_IN ("pri_proc [idle enter execs]", state1_enter_exec)
				{
				if(END)
				{
				/*
			...
			FSM_STATE_EXIT_UNFORCED (1, "idle", "pri_proc [idle exit execs]")
				FSM_PROFILE_SECTION_IN ("pri_proc [idle exit execs]", state1_exit_exec)
				{
				/*
				if(node_type!=sink)
					grade=1;
			...
			FSM_PROFILE_SECTION_IN ("pri_proc [idle trans conditions]", state1_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 6, state6_enter_exec, ;, "CONTEND_TO_SEND_RTS", "", "idle", "second contention", "tr_28", "pri_proc [idle -> second contention : CONTEND_TO_SEND_RTS / ]")
				FSM_CASE_TRANSIT (1, 4, state4_enter_exec, ;, "GO_TO_SLEEP", "", "idle", "sleeping", "tr_32", "pri_proc [idle -> sleeping : GO_TO_SLEEP / ]")
				FSM_CASE_TRANSIT (2, 5, state5_enter_exec, ;, "WAKING_TO_WAIT_RTS", "", "idle", "waking", "tr_34", "pri_proc [idle -> waking : WAKING_TO_WAIT_RTS / ]")
				FSM_CASE_TRANSIT (3, 17, state17_enter_exec, ;, "START_GRADE", "", "idle", "start grade", "tr_97", "pri_proc [idle -> start grade : START_GRADE / ]")
				FSM_CASE_TRANSIT (4, 3, state3_enter_exec, ;, "SEND_GM", "", "idle", "send gm", "tr_100", "pri_proc [idle -> send gm : SEND_GM / ]")
				FSM_CASE_TRANSIT (5, 2, state2_enter_exec, ;, "FROM_LOWER && transceiver_state", "", "idle", "GM proc", "tr_102", "pri_proc [idle -> GM proc : FROM_LOWER && transceiver_state / ]")
				FSM_CASE_TRANSIT (6, 1, state1_enter_exec, ;, "default", "", "idle", "idle", "tr_108", "pri_proc [idle -> idle : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_FORCED (2, "GM proc", state2_enter_exec, "pri_proc [GM proc enter execs]")
				FSM_PROFILE_SECTION_IN ("pri_proc [GM proc enter execs]", state2_enter_exec)
				{
			...
			FSM_STATE_EXIT_FORCED (2, "GM proc", "pri_proc [GM proc exit execs]")


			/** state (GM proc) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "GM proc", "idle", "tr_103", "pri_proc [GM proc -> idle : default / ]")
				/*---------------------------------------------------------*/



			/** state (send gm) enter executives **/
			FSM_STATE_ENTER_FORCED (3, "send gm", state3_enter_exec, "pri_proc [send gm enter execs]")
				FSM_PROFILE_SECTION_IN ("pri_proc [send gm enter execs]", state3_enter_exec)
				{
				if(op_subq_empty(grade_queue) == OPC_FALSE)
				{
			...
			FSM_STATE_EXIT_FORCED (3, "send gm", "pri_proc [send gm exit execs]")


			/** state (send gm) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "send gm", "idle", "tr_101", "pri_proc [send gm -> idle : default / ]")
				/*---------------------------------------------------------*/



			/** state (sleeping) enter executives **/
			FSM_STATE_ENTER_UNFORCED (4, "sleeping", state4_enter_exec, "pri_proc [sleeping enter execs]")
				FSM_PROFILE_SECTION_IN ("pri_proc [sleeping enter execs]", state4_enter_exec)
				{
			...
			FSM_STATE_EXIT_UNFORCED (4, "sleeping", "pri_proc [sleeping exit execs]")


			/** state (sleeping) transition processing **/
			FSM_PROFILE_SECTION_IN ("pri_proc [sleeping trans conditions]", state4_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 5, state5_enter_exec, ;, "WAKING_UP", "", "sleeping", "waking", "tr_37", "pri_proc [sleeping -> waking : WAKING_UP / ]")
				FSM_CASE_TRANSIT (1, 6, state6_enter_exec, ;, "WAKING_TO_SEND_RTS", "", "sleeping", "second contention", "tr_40", "pri_proc [sleeping -> second contention : WAKING_TO_SEND_RTS / ]")
				FSM_CASE_TRANSIT (2, 4, state4_enter_exec, ;, "default", "", "sleeping", "sleeping", "tr_91", "pri_proc [sleeping -> sleeping : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_FORCED (5, "waking", state5_enter_exec, "pri_proc [waking enter execs]")
				FSM_PROFILE_SECTION_IN ("pri_proc [waking enter execs]", state5_enter_exec)
				{
				//The number of packets in the queue at the beginning of R state.
				op_stat_write(QLength_handle, op_subq_stat(data_queue,OPC_QSTAT_PKSIZE));
			...
			FSM_STATE_EXIT_FORCED (5, "waking", "pri_proc [waking exit execs]")


			/** state (waking) transition processing **/
			FSM_TRANSIT_FORCE (8, state8_enter_exec, ;, "default", "", "waking", "first", "tr_57", "pri_proc [waking -> first : default / ]")
				/*---------------------------------------------------------*/



			/** state (second contention) enter executives **/
			FSM_STATE_ENTER_FORCED (6, "second contention", state6_enter_exec, "pri_proc [second contention enter execs]")
				FSM_PROFILE_SECTION_IN ("pri_proc [second contention enter execs]", state6_enter_exec)
				{
			...
			FSM_STATE_EXIT_FORCED (6, "second contention", "pri_proc [second contention exit execs]")


			/** state (second contention) transition processing **/
			FSM_TRANSIT_FORCE (9, state9_enter_exec, ;, "default", "", "second contention", "second", "tr_59", "pri_proc [second contention -> second : default / ]")
				/*---------------------------------------------------------*/



			/** state (wait CTS) enter executives **/
			FSM_STATE_ENTER_FORCED (7, "wait CTS", state7_enter_exec, "pri_proc [wait CTS enter execs]")
				FSM_PROFILE_SECTION_IN ("pri_proc [wait CTS enter execs]", state7_enter_exec)
				{
				//send RTS
				if(pk_TV!=NULL)
				{
			...
			FSM_STATE_EXIT_FORCED (7, "wait CTS", "pri_proc [wait CTS exit execs]")


			/** state (wait CTS) transition processing **/
			FSM_TRANSIT_FORCE (9, state9_enter_exec, ;, "default", "", "wait CTS", "second", "tr_89", "pri_proc [wait CTS -> second : default / ]")
				/*---------------------------------------------------------*/



			/** state (first) enter executives **/
			FSM_STATE_ENTER_UNFORCED (8, "first", state8_enter_exec, "pri_proc [first enter execs]")
				FSM_PROFILE_SECTION_IN ("pri_proc [first enter execs]", state8_enter_exec)
				{
				//Received DATA from upper layer
				if(FROM_UPPER)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (17,"pri_proc")


			/** state (first) exit executives **/
			FSM_STATE_EXIT_UNFORCED (8, "first", "pri_proc [first exit execs]")


			/** state (first) transition processing **/
			FSM_PROFILE_SECTION_IN ("pri_proc [first trans conditions]", state8_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 4, state4_enter_exec, ;, "SLEEP_FROM_FIRST", "", "first", "sleeping", "tr_56", "pri_proc [first -> sleeping : SLEEP_FROM_FIRST / ]")
				FSM_CASE_TRANSIT (1, 12, state12_enter_exec, ;, "FROM_LOWER && transceiver_state", "", "first", "RCV(1)", "tr_74", "pri_proc [first -> RCV(1) : FROM_LOWER && transceiver_state / ]")
				FSM_CASE_TRANSIT (2, 13, state13_enter_exec, ;, "SEND_CTS", "", "first", "wait DATA", "tr_76", "pri_proc [first -> wait DATA : SEND_CTS / ]")
				FSM_CASE_TRANSIT (3, 14, state14_enter_exec, ;, "SEND_ACK", "", "first", "send ACK", "tr_79", "pri_proc [first -> send ACK : SEND_ACK / ]")
				FSM_CASE_TRANSIT (4, 15, state15_enter_exec, ;, "CHECK_CHANNEL", "", "first", "carrier sense(1)", "tr_93", "pri_proc [first -> carrier sense(1) : CHECK_CHANNEL / ]")
				FSM_CASE_TRANSIT (5, 8, state8_enter_exec, ;, "default", "", "first", "first", "tr_90", "pri_proc [first -> first : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_UNFORCED (9, "second", state9_enter_exec, "pri_proc [second enter execs]")
				FSM_PROFILE_SECTION_IN ("pri_proc [second enter execs]", state9_enter_exec)
				{
				//Received DATA from upper layer
				if(FROM_UPPER)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (19,"pri_proc")


			/** state (second) exit executives **/
			FSM_STATE_EXIT_UNFORCED (9, "second", "pri_proc [second exit execs]")


			/** state (second) transition processing **/
			FSM_PROFILE_SECTION_IN ("pri_proc [second trans conditions]", state9_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 7, state7_enter_exec, ;, "SEND_RTS", "", "second", "wait CTS", "tr_60", "pri_proc [second -> wait CTS : SEND_RTS / ]")
				FSM_CASE_TRANSIT (1, 4, state4_enter_exec, ;, "SLEEP_FROM_SECOND", "", "second", "sleeping", "tr_61", "pri_proc [second -> sleeping : SLEEP_FROM_SECOND / ]")
				FSM_CASE_TRANSIT (2, 10, state10_enter_exec, ;, "FROM_LOWER && transceiver_state", "", "second", "RCV(2)", "tr_66", "pri_proc [second -> RCV(2) : FROM_LOWER && transceiver_state / ]")
				FSM_CASE_TRANSIT (3, 11, state11_enter_exec, ;, "SEND_DATA", "", "second", "send data", "tr_72", "pri_proc [second -> send data : SEND_DATA / ]")
				FSM_CASE_TRANSIT (4, 16, state16_enter_exec, ;, "CHECK_CHANNEL", "", "second", "carrier sense(2)", "tr_95", "pri_proc [second -> carrier sense(2) : CHECK_CHANNEL / ]")
				FSM_CASE_TRANSIT (5, 9, state9_enter_exec, ;, "default", "", "second", "second", "tr_92", "pri_proc [second -> second : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_FORCED (10, "RCV(2)", state10_enter_exec, "pri_proc [RCV(2) enter execs]")
				FSM_PROFILE_SECTION_IN ("pri_proc [RCV(2) enter execs]", state10_enter_exec)
				{
				if(pk_TV!=NULL)
				{
					op_pk_destroy(pk_TV);
			...
			FSM_STATE_EXIT_FORCED (10, "RCV(2)", "pri_proc [RCV(2) exit execs]")


			/** state (RCV(2)) transition processing **/
			FSM_TRANSIT_FORCE (9, state9_enter_exec, ;, "default", "", "RCV(2)", "second", "tr_71", "pri_proc [RCV(2) -> second : default / ]")
				/*---------------------------------------------------------*/



			/** state (send data) enter executives **/
			FSM_STATE_ENTER_FORCED (11, "send data", state11_enter_exec, "pri_proc [send data enter execs]")
				FSM_PROFILE_SECTION_IN ("pri_proc [send data enter execs]", state11_enter_exec)
				{
				//send DATA
				if(pk_TV!=NULL)
				{
			...
			FSM_STATE_EXIT_FORCED (11, "send data", "pri_proc [send data exit execs]")


			/** state (send data) transition processing **/
			FSM_TRANSIT_FORCE (9, state9_enter_exec, ;, "default", "", "send data", "second", "tr_73", "pri_proc [send data -> second : default / ]")
				/*---------------------------------------------------------*/



			/** state (RCV(1)) enter executives **/
			FSM_STATE_ENTER_FORCED (12, "RCV(1)", state12_enter_exec, "pri_proc [RCV(1) enter execs]")
				FSM_PROFILE_SECTION_IN ("pri_proc [RCV(1) enter execs]", state12_enter_exec)
				{
				if(pk_TV!=NULL)
				{
					op_pk_destroy(pk_TV);
			...
			FSM_STATE_EXIT_FORCED (12, "RCV(1)", "pri_proc [RCV(1) exit execs]")


			/** state (RCV(1)) transition processing **/
			FSM_TRANSIT_FORCE (8, state8_enter_exec, ;, "default", "", "RCV(1)", "first", "tr_75", "pri_proc [RCV(1) -> first : default / ]")
				/*---------------------------------------------------------*/



			/** state (wait DATA) enter executives **/
			FSM_STATE_ENTER_FORCED (13, "wait DATA", state13_enter_exec, "pri_proc [wait DATA enter execs]")
				FSM_PROFILE_SECTION_IN ("pri_proc [wait DATA enter execs]", state13_enter_exec)
				{
				//send CTS
				if(pk_TV!=NULL)
				{
			...
			FSM_STATE_EXIT_FORCED (13, "wait DATA", "pri_proc [wait DATA exit execs]")


			/** state (wait DATA) transition processing **/
			FSM_TRANSIT_FORCE (8, state8_enter_exec, ;, "default", "", "wait DATA", "first", "tr_77", "pri_proc [wait DATA -> first : default / ]")
				/*---------------------------------------------------------*/



			/** state (send ACK) enter executives **/
			FSM_STATE_ENTER_FORCED (14, "send ACK", state14_enter_exec, "pri_proc [send ACK enter execs]")
				FSM_PROFILE_SECTION_IN ("pri_proc [send ACK enter execs]", state14_enter_exec)
				{
				//send ACK
				if(pk_TV!=NULL)
				{
			...
			FSM_STATE_EXIT_FORCED (14, "send ACK", "pri_proc [send ACK exit execs]")


			/** state (send ACK) transition processing **/
			FSM_TRANSIT_FORCE (8, state8_enter_exec, ;, "default", "", "send ACK", "first", "tr_80", "pri_proc [send ACK -> first : default / ]")
				/*---------------------------------------------------------*/



			/** state (carrier sense(1)) enter executives **/
			FSM_STATE_ENTER_FORCED (15, "carrier sense(1)", state15_enter_exec, "pri_proc [carrier sense(1) enter execs]")
				FSM_PROFILE_SECTION_IN ("pri_proc [carrier sense(1) enter execs]", state15_enter_exec)
				{
				if(debug)
				{
			...
			FSM_STATE_EXIT_FORCED (15, "carrier sense(1)", "pri_proc [carrier sense(1) exit execs]")


			/** state (carrier sense(1)) transition processing **/
			FSM_TRANSIT_FORCE (8, state8_enter_exec, ;, "default", "", "carrier sense(1)", "first", "tr_94", "pri_proc [carrier sense(1) -> first : default / ]")
				/*---------------------------------------------------------*/



			/** state (carrier sense(2)) enter executives **/
			FSM_STATE_ENTER_FORCED (16, "carrier sense(2)", state16_enter_exec, "pri_proc [carrier sense(2) enter execs]")
				FSM_PROFILE_SECTION_IN ("pri_proc [carrier sense(2) enter execs]", state16_enter_exec)
				{
				if(debug)
			...
			FSM_STATE_EXIT_FORCED (16, "carrier sense(2)", "pri_proc [carrier sense(2) exit execs]")


			/** state (carrier sense(2)) transition processing **/
			FSM_TRANSIT_FORCE (9, state9_enter_exec, ;, "default", "", "carrier sense(2)", "second", "tr_96", "pri_proc [carrier sense(2) -> second : default / ]")
				/*---------------------------------------------------------*/



			/** state (start grade) enter executives **/
			FSM_STATE_ENTER_FORCED (17, "start grade", state17_enter_exec, "pri_proc [start grade enter execs]")
				FSM_PROFILE_SECTION_IN ("pri_proc [start grade enter execs]", state17_enter_exec)
				{
			...
			FSM_STATE_EXIT_FORCED (17, "start grade", "pri_proc [start grade exit execs]")


			/** state (start grade) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "start grade", "idle", "tr_98", "pri_proc [start grade -> idle : default / ]")
				/*---------------------------------------------------------*/
...
void
_op_pri_proc_diag (OP_SIM_CONTEXT_ARG_OPT)
	{
	/* No Diagnostic Block */
	}
...
void
_op_pri_proc_terminate (OP_SIM_CONTEXT_ARG_OPT)
	{

	FIN_MT (_op_pri_proc_terminate ())
...
/* local variable prs_ptr in _op_pri_proc_svar function. */
#undef process_id
#undef node_id
#undef node_address
#undef grade
...
VosT_Obtype
_op_pri_proc_init (int * init_block_ptr)
	{
	VosT_Obtype obtype = OPC_NIL;
	FIN_MT (_op_pri_proc_init (init_block_ptr))

	obtype = Vos_Define_Object_Prstate ("proc state vars (pri_proc)",
		sizeof (pri_proc_state));
	*init_block_ptr = 0;
...
VosT_Address
_op_pri_proc_alloc (VosT_Obtype obtype, int init_block)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	pri_proc_state * ptr;
	FIN_MT (_op_pri_proc_alloc (obtype))

	ptr = (pri_proc_state *)Vos_Alloc_Object (obtype);
	if (ptr != OPC_NIL)
		{
		ptr->_op_current_block = init_block;
#if defined (OPD_ALLOW_ODB)
		ptr->_op_current_state = "pri_proc [init enter execs]";
#endif
		}
...
void
_op_pri_proc_svar (void * gen_ptr, const char * var_name, void ** var_p_ptr)
	{
	pri_proc_state		*prs_ptr;

	FIN_MT (_op_pri_proc_svar (gen_ptr, var_name, var_p_ptr))
	...
	prs_ptr = (pri_proc_state *)gen_ptr;
