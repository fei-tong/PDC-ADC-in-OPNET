/* Process model C form file: pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1.pr.c */
/* Portions of this file copyright 1986-2008 by OPNET Technologies, Inc. */



/* This variable carries the header into the object file */
const char pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1_pr_c [] = "MIL_3_Tfile_Hdr_ 145A 30A modeler 7 52F3CB9C 52F3CB9C 1 wpd FeiTong 0 0 none none 0 0 none 0 0 0 0 0 0 0 0 1e80 8                                                                                                                                                                                                                                                                                                                                                                                                              ";
#include <string.h>
...
typedef struct
	...
	double	                 		steady_time                                     ;
	List *	                 		sink_neighbor_list                              ;
	Evhandle	               		regular_wakeup_evhandle                         ;	/* 正常（不是在sleeping中醒来）醒来事件句柄设置 */
	int	                    		sv_current_slot                                 ;	/* 记录当前sleeping slot的位置，between [1,sleep factor] */
	...
	double	                 		sent_pkts_num                                   ;	/* to calculate the average queueing delay */
	double	                 		sum_src_delay                                   ;
	double	                 		src_pkt_num                                     ;
	double	                 		sum_relay_delay                                 ;
	double	                 		relay_pkt_num                                   ;
	} pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1_state;
...
#define sink_neighbor_list      		op_sv_ptr->sink_neighbor_list
...
#define sent_pkts_num           		op_sv_ptr->sent_pkts_num
...
#define FIN_PREAMBLE_DEC	pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1_state *op_sv_ptr;
#define FIN_PREAMBLE_CODE	\
		op_sv_ptr = ((pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr));
...
//body
	if(HRC_ON){
		if(node_type == sink)// the assignment is done by the sink
		{
			...
			slot_num=sleep_factor-2;//the last two slots in a cycle (RTS) cannot be used for interference consideration
...
//body
	if(HRC_ON){
		if(node_type == sink)// the assignment is done by the sink
		{
			...
			slot_pos=1;
			for(list_i=0;list_i<neighbor_num && slot_pos<slot_num;list_i++)//有left个节点分到(1+remainder)个slots，
			{
				sink_neighbor=(SinkNeighbor *)op_prg_list_access(sink_neighbor_list,list_i);
				sink_neighbor->start_slot = slot_pos;
				sink_neighbor->num_of_slots = 1+1;
				slot_pos = slot_pos + 4;
			}
		}
	}
...
//body
	...
	if(total_send_trial!=0.0){
		op_stat_write_t(send_prob_handle,win_success/total_send_trial,node_address);
		op_stat_write_t(send_prob_handle,send_success/total_send_trial,node_address+10);
	}
...
extern "C" {
#endif
	void pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Obtype _op_pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1_init (int * init_block_ptr);
	void _op_pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1_diag (OP_SIM_CONTEXT_ARG_OPT);
	void _op_pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1_terminate (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Address _op_pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1_alloc (VosT_Obtype, int);
	void _op_pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1_svar (void *, const char *, void **);
...
void
pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 (OP_SIM_CONTEXT_ARG_OPT)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	FIN_MT (pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 ());
		...
		FSM_ENTER ("pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1")

		FSM_BLOCK_SWITCH
			{
			/*---------------------------------------------------------*/
			/** state (init) enter executives **/
			FSM_STATE_ENTER_FORCED_NOLABEL (0, "init", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [init enter execs]")
				FSM_PROFILE_SECTION_IN ("pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [init enter execs]", state0_enter_exec)
				{
				// Obtain related ID
				...
				send_prob_handle = op_stat_reg("Win/Send Probability", OPC_STAT_INDEX_NONE, OPC_STAT_LOCAL);
				receive_prob_handle = op_stat_reg("Receive Probability", OPC_STAT_INDEX_NONE, OPC_STAT_LOCAL);
			...
			FSM_STATE_EXIT_FORCED (0, "init", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [init exit execs]")
				FSM_PROFILE_SECTION_IN ("pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [init exit execs]", state0_exit_exec)
				{
				/*
			...
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "init", "idle", "tr_1", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [init -> idle : default / ]")
				/*---------------------------------------------------------*/



			/** state (idle) enter executives **/
			FSM_STATE_ENTER_UNFORCED (1, "idle", state1_enter_exec, "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [idle enter execs]")
				FSM_PROFILE_SECTION_IN ("pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [idle enter execs]", state1_enter_exec)
				{
				///*
				if(CHECK_CHANNEL){
					if(op_ev_valid(send_gm_event)){
			...
			FSM_EXIT (3,"pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1")


			/** state (idle) exit executives **/
			FSM_STATE_EXIT_UNFORCED (1, "idle", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [idle exit execs]")
				FSM_PROFILE_SECTION_IN ("pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [idle exit execs]", state1_exit_exec)
				{
				/*
				if(node_type!=sink)
					grade=1;
			...
			FSM_PROFILE_SECTION_IN ("pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [idle trans conditions]", state1_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 6, state6_enter_exec, slot_assignment();sv_current_slot=0;;, "CONTEND_TO_SEND_RTS", "slot_assignment();sv_current_slot=0;", "idle", "second contention", "tr_28", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [idle -> second contention : CONTEND_TO_SEND_RTS / slot_assignment();sv_current_slot=0;]")
				FSM_CASE_TRANSIT (1, 4, state4_enter_exec, slot_assignment();, "GO_TO_SLEEP", "slot_assignment()", "idle", "sleeping", "tr_32", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [idle -> sleeping : GO_TO_SLEEP / slot_assignment()]")
				FSM_CASE_TRANSIT (2, 5, state5_enter_exec, slot_assignment();sv_current_slot=-1;;, "WAKING_TO_WAIT_RTS", "slot_assignment();sv_current_slot=-1;", "idle", "waking", "tr_34", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [idle -> waking : WAKING_TO_WAIT_RTS / slot_assignment();sv_current_slot=-1;]")
				FSM_CASE_TRANSIT (3, 17, state17_enter_exec, ;, "START_GRADE", "", "idle", "start grade", "tr_97", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [idle -> start grade : START_GRADE / ]")
				FSM_CASE_TRANSIT (4, 3, state3_enter_exec, ;, "SEND_GM", "", "idle", "send gm", "tr_100", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [idle -> send gm : SEND_GM / ]")
				FSM_CASE_TRANSIT (5, 2, state2_enter_exec, ;, "FROM_LOWER && transceiver_state", "", "idle", "GM proc", "tr_102", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [idle -> GM proc : FROM_LOWER && transceiver_state / ]")
				FSM_CASE_TRANSIT (6, 1, state1_enter_exec, ;, "default", "", "idle", "idle", "tr_108", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [idle -> idle : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_FORCED (2, "GM proc", state2_enter_exec, "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [GM proc enter execs]")
				FSM_PROFILE_SECTION_IN ("pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [GM proc enter execs]", state2_enter_exec)
				{
			...
			FSM_STATE_EXIT_FORCED (2, "GM proc", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [GM proc exit execs]")


			/** state (GM proc) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "GM proc", "idle", "tr_103", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [GM proc -> idle : default / ]")
				/*---------------------------------------------------------*/



			/** state (send gm) enter executives **/
			FSM_STATE_ENTER_FORCED (3, "send gm", state3_enter_exec, "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [send gm enter execs]")
				FSM_PROFILE_SECTION_IN ("pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [send gm enter execs]", state3_enter_exec)
				{
				if(op_subq_empty(grade_queue) == OPC_FALSE)
				{
			...
			FSM_STATE_EXIT_FORCED (3, "send gm", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [send gm exit execs]")


			/** state (send gm) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "send gm", "idle", "tr_101", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [send gm -> idle : default / ]")
				/*---------------------------------------------------------*/



			/** state (sleeping) enter executives **/
			FSM_STATE_ENTER_UNFORCED (4, "sleeping", state4_enter_exec, "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [sleeping enter execs]")
				FSM_PROFILE_SECTION_IN ("pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [sleeping enter execs]", state4_enter_exec)
				{
				...
				if(op_intrpt_code() == SLEEP_FROM_FIRST_CODE)//sleep from first period,i.e.,R state
				{
					...
					if (node_type == sensor){
						...
						if(HRC_ON && grade==2){// && mac_flags->ddc_relay == OPC_TRUE && sv_current_slot !=-1){
							
							if(DDC_ON && mac_flags->ddc_relay == OPC_TRUE && ddc_sleep_factor>=6){
							
								time = waking_time + durActive_half + 3*durActive_half - op_sim_time();
								iciptr_TV = op_ici_create("slot_ici");
				...
				if(op_intrpt_code() == SLEEP_FROM_SECOND_CODE)//sleep from second period,i.e.,T state
				{
					...
					if (LHT_ON){
						if(grade == 1){
							if((sleep_factor-sv_current_slot) >2 ){//需要再做一次判断，防止节点没有进入"wait CTS". 本判断表明sleep中还有slot可以利用
								if(op_subq_stat(data_queue,OPC_QSTAT_PKSIZE)>0){
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (9,"pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1")


			/** state (sleeping) exit executives **/
			FSM_STATE_EXIT_UNFORCED (4, "sleeping", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [sleeping exit execs]")


			/** state (sleeping) transition processing **/
			FSM_PROFILE_SECTION_IN ("pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [sleeping trans conditions]", state4_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 5, state5_enter_exec, ;, "WAKING_UP", "", "sleeping", "waking", "tr_37", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [sleeping -> waking : WAKING_UP / ]")
				FSM_CASE_TRANSIT (1, 6, state6_enter_exec, ;, "WAKING_TO_SEND_RTS", "", "sleeping", "second contention", "tr_40", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [sleeping -> second contention : WAKING_TO_SEND_RTS / ]")
				FSM_CASE_TRANSIT (2, 4, state4_enter_exec, ;, "default", "", "sleeping", "sleeping", "tr_91", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [sleeping -> sleeping : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_FORCED (5, "waking", state5_enter_exec, "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [waking enter execs]")
				FSM_PROFILE_SECTION_IN ("pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [waking enter execs]", state5_enter_exec)
				{
				if(WAKING_UP){
					if(node_type==sensor)
					{
				...
				if(op_sim_time() >= steady_time) {total_receive_trial++;}
			...
			FSM_STATE_EXIT_FORCED (5, "waking", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [waking exit execs]")


			/** state (waking) transition processing **/
			FSM_TRANSIT_FORCE (8, state8_enter_exec, ;, "default", "", "waking", "first", "tr_57", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [waking -> first : default / ]")
				/*---------------------------------------------------------*/



			/** state (second contention) enter executives **/
			FSM_STATE_ENTER_FORCED (6, "second contention", state6_enter_exec, "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [second contention enter execs]")
				FSM_PROFILE_SECTION_IN ("pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [second contention enter execs]", state6_enter_exec)
				{
			...
			FSM_STATE_EXIT_FORCED (6, "second contention", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [second contention exit execs]")


			/** state (second contention) transition processing **/
			FSM_TRANSIT_FORCE (9, state9_enter_exec, ;, "default", "", "second contention", "second", "tr_59", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [second contention -> second : default / ]")
				/*---------------------------------------------------------*/



			/** state (wait CTS) enter executives **/
			FSM_STATE_ENTER_FORCED (7, "wait CTS", state7_enter_exec, "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [wait CTS enter execs]")
				FSM_PROFILE_SECTION_IN ("pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [wait CTS enter execs]", state7_enter_exec)
				{
				//send RTS
				if(pk_TV!=NULL)
				{
			...
			FSM_STATE_EXIT_FORCED (7, "wait CTS", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [wait CTS exit execs]")


			/** state (wait CTS) transition processing **/
			FSM_TRANSIT_FORCE (9, state9_enter_exec, ;, "default", "", "wait CTS", "second", "tr_89", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [wait CTS -> second : default / ]")
				/*---------------------------------------------------------*/



			/** state (first) enter executives **/
			FSM_STATE_ENTER_UNFORCED (8, "first", state8_enter_exec, "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [first enter execs]")
				FSM_PROFILE_SECTION_IN ("pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [first enter execs]", state8_enter_exec)
				{
				//Received DATA from upper layer
				if(FROM_UPPER)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (17,"pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1")


			/** state (first) exit executives **/
			FSM_STATE_EXIT_UNFORCED (8, "first", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [first exit execs]")


			/** state (first) transition processing **/
			FSM_PROFILE_SECTION_IN ("pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [first trans conditions]", state8_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 4, state4_enter_exec, ;, "SLEEP_FROM_FIRST", "", "first", "sleeping", "tr_56", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [first -> sleeping : SLEEP_FROM_FIRST / ]")
				FSM_CASE_TRANSIT (1, 12, state12_enter_exec, ;, "FROM_LOWER && transceiver_state", "", "first", "RCV(1)", "tr_74", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [first -> RCV(1) : FROM_LOWER && transceiver_state / ]")
				FSM_CASE_TRANSIT (2, 13, state13_enter_exec, ;, "SEND_CTS", "", "first", "wait DATA", "tr_76", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [first -> wait DATA : SEND_CTS / ]")
				FSM_CASE_TRANSIT (3, 14, state14_enter_exec, ;, "SEND_ACK", "", "first", "send ACK", "tr_79", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [first -> send ACK : SEND_ACK / ]")
				FSM_CASE_TRANSIT (4, 15, state15_enter_exec, ;, "CHECK_CHANNEL", "", "first", "carrier sense(1)", "tr_93", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [first -> carrier sense(1) : CHECK_CHANNEL / ]")
				FSM_CASE_TRANSIT (5, 8, state8_enter_exec, ;, "default", "", "first", "first", "tr_90", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [first -> first : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_UNFORCED (9, "second", state9_enter_exec, "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [second enter execs]")
				FSM_PROFILE_SECTION_IN ("pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [second enter execs]", state9_enter_exec)
				{
				//Received DATA from upper layer
				if(FROM_UPPER)
				{
				...
				if(DECREASE_CW){
					if(mac_flags->busy_before_rts == OPC_TRUE){
						...
						if(op_subq_empty(data_queue) == OPC_TRUE){
							...
							if(op_sim_time()>=steady_time) {win_success++;}
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (19,"pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1")


			/** state (second) exit executives **/
			FSM_STATE_EXIT_UNFORCED (9, "second", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [second exit execs]")


			/** state (second) transition processing **/
			FSM_PROFILE_SECTION_IN ("pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [second trans conditions]", state9_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 7, state7_enter_exec, ;, "SEND_RTS", "", "second", "wait CTS", "tr_60", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [second -> wait CTS : SEND_RTS / ]")
				FSM_CASE_TRANSIT (1, 4, state4_enter_exec, ;, "SLEEP_FROM_SECOND", "", "second", "sleeping", "tr_61", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [second -> sleeping : SLEEP_FROM_SECOND / ]")
				FSM_CASE_TRANSIT (2, 10, state10_enter_exec, ;, "FROM_LOWER && transceiver_state", "", "second", "RCV(2)", "tr_66", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [second -> RCV(2) : FROM_LOWER && transceiver_state / ]")
				FSM_CASE_TRANSIT (3, 11, state11_enter_exec, ;, "SEND_DATA", "", "second", "send data", "tr_72", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [second -> send data : SEND_DATA / ]")
				FSM_CASE_TRANSIT (4, 16, state16_enter_exec, ;, "CHECK_CHANNEL", "", "second", "carrier sense(2)", "tr_95", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [second -> carrier sense(2) : CHECK_CHANNEL / ]")
				FSM_CASE_TRANSIT (5, 9, state9_enter_exec, ;, "default", "", "second", "second", "tr_92", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [second -> second : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_FORCED (10, "RCV(2)", state10_enter_exec, "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [RCV(2) enter execs]")
				FSM_PROFILE_SECTION_IN ("pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [RCV(2) enter execs]", state10_enter_exec)
				{
				if(pk_TV!=NULL)
				{
					op_pk_destroy(pk_TV);
			...
			FSM_STATE_EXIT_FORCED (10, "RCV(2)", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [RCV(2) exit execs]")


			/** state (RCV(2)) transition processing **/
			FSM_TRANSIT_FORCE (9, state9_enter_exec, ;, "default", "", "RCV(2)", "second", "tr_71", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [RCV(2) -> second : default / ]")
				/*---------------------------------------------------------*/



			/** state (send data) enter executives **/
			FSM_STATE_ENTER_FORCED (11, "send data", state11_enter_exec, "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [send data enter execs]")
				FSM_PROFILE_SECTION_IN ("pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [send data enter execs]", state11_enter_exec)
				{
				//send DATA
				if(pk_TV!=NULL)
				{
			...
			FSM_STATE_EXIT_FORCED (11, "send data", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [send data exit execs]")


			/** state (send data) transition processing **/
			FSM_TRANSIT_FORCE (9, state9_enter_exec, ;, "default", "", "send data", "second", "tr_73", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [send data -> second : default / ]")
				/*---------------------------------------------------------*/



			/** state (RCV(1)) enter executives **/
			FSM_STATE_ENTER_FORCED (12, "RCV(1)", state12_enter_exec, "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [RCV(1) enter execs]")
				FSM_PROFILE_SECTION_IN ("pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [RCV(1) enter execs]", state12_enter_exec)
				{
				if(pk_TV!=NULL)
				{
					op_pk_destroy(pk_TV);
					...
					if(grade_TV == grade+1)
					{
						if(op_ev_valid(wait_RTS_event))//the first time to receive RTS
						{
							...
							if(one_two_cw == 2){
								...
								if(HRC_ON){
									if(grade == 1){ //grade-one nodes doesn't need to relay packet from grade-two nodes.
										mac_flags->ddc_relay=OPC_FALSE;
									}
								}
			...
			FSM_STATE_EXIT_FORCED (12, "RCV(1)", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [RCV(1) exit execs]")


			/** state (RCV(1)) transition processing **/
			FSM_TRANSIT_FORCE (8, state8_enter_exec, ;, "default", "", "RCV(1)", "first", "tr_75", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [RCV(1) -> first : default / ]")
				/*---------------------------------------------------------*/



			/** state (wait DATA) enter executives **/
			FSM_STATE_ENTER_FORCED (13, "wait DATA", state13_enter_exec, "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [wait DATA enter execs]")
				FSM_PROFILE_SECTION_IN ("pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [wait DATA enter execs]", state13_enter_exec)
				{
				//send CTS
				if(pk_TV!=NULL)
				{
			...
			FSM_STATE_EXIT_FORCED (13, "wait DATA", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [wait DATA exit execs]")


			/** state (wait DATA) transition processing **/
			FSM_TRANSIT_FORCE (8, state8_enter_exec, ;, "default", "", "wait DATA", "first", "tr_77", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [wait DATA -> first : default / ]")
				/*---------------------------------------------------------*/



			/** state (send ACK) enter executives **/
			FSM_STATE_ENTER_FORCED (14, "send ACK", state14_enter_exec, "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [send ACK enter execs]")
				FSM_PROFILE_SECTION_IN ("pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [send ACK enter execs]", state14_enter_exec)
				{
				//send ACK
				if(pk_TV!=NULL)
				{
			...
			FSM_STATE_EXIT_FORCED (14, "send ACK", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [send ACK exit execs]")


			/** state (send ACK) transition processing **/
			FSM_TRANSIT_FORCE (8, state8_enter_exec, ;, "default", "", "send ACK", "first", "tr_80", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [send ACK -> first : default / ]")
				/*---------------------------------------------------------*/



			/** state (carrier sense(1)) enter executives **/
			FSM_STATE_ENTER_FORCED (15, "carrier sense(1)", state15_enter_exec, "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [carrier sense(1) enter execs]")
				FSM_PROFILE_SECTION_IN ("pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [carrier sense(1) enter execs]", state15_enter_exec)
				{
				if(DEBUG)
				{
			...
			FSM_STATE_EXIT_FORCED (15, "carrier sense(1)", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [carrier sense(1) exit execs]")


			/** state (carrier sense(1)) transition processing **/
			FSM_TRANSIT_FORCE (8, state8_enter_exec, ;, "default", "", "carrier sense(1)", "first", "tr_94", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [carrier sense(1) -> first : default / ]")
				/*---------------------------------------------------------*/



			/** state (carrier sense(2)) enter executives **/
			FSM_STATE_ENTER_FORCED (16, "carrier sense(2)", state16_enter_exec, "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [carrier sense(2) enter execs]")
				FSM_PROFILE_SECTION_IN ("pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [carrier sense(2) enter execs]", state16_enter_exec)
				{
				if(DEBUG)
			...
			FSM_STATE_EXIT_FORCED (16, "carrier sense(2)", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [carrier sense(2) exit execs]")


			/** state (carrier sense(2)) transition processing **/
			FSM_TRANSIT_FORCE (9, state9_enter_exec, ;, "default", "", "carrier sense(2)", "second", "tr_96", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [carrier sense(2) -> second : default / ]")
				/*---------------------------------------------------------*/



			/** state (start grade) enter executives **/
			FSM_STATE_ENTER_FORCED (17, "start grade", state17_enter_exec, "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [start grade enter execs]")
				FSM_PROFILE_SECTION_IN ("pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [start grade enter execs]", state17_enter_exec)
				{
			...
			FSM_STATE_EXIT_FORCED (17, "start grade", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [start grade exit execs]")


			/** state (start grade) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "start grade", "idle", "tr_98", "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [start grade -> idle : default / ]")
				/*---------------------------------------------------------*/
		...
		FSM_EXIT (0,"pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1")
		}
	}
...
void
_op_pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1_diag (OP_SIM_CONTEXT_ARG_OPT)
	{
	/* No Diagnostic Block */
	}
...
void
_op_pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1_terminate (OP_SIM_CONTEXT_ARG_OPT)
	{

	FIN_MT (_op_pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1_terminate ())
...
/* local variable prs_ptr in _op_pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1_svar function. */
#undef process_id
#undef node_id
#undef node_address
#undef grade
...
VosT_Obtype
_op_pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1_init (int * init_block_ptr)
	{
	VosT_Obtype obtype = OPC_NIL;
	FIN_MT (_op_pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1_init (init_block_ptr))

	obtype = Vos_Define_Object_Prstate ("proc state vars (pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1)",
		sizeof (pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1_state));
	*init_block_ptr = 0;
...
VosT_Address
_op_pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1_alloc (VosT_Obtype obtype, int init_block)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1_state * ptr;
	FIN_MT (_op_pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1_alloc (obtype))

	ptr = (pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1_state *)Vos_Alloc_Object (obtype);
	if (ptr != OPC_NIL)
		{
		ptr->_op_current_block = init_block;
#if defined (OPD_ALLOW_ODB)
		ptr->_op_current_state = "pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1 [init enter execs]";
#endif
		}
...
void
_op_pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1_svar (void * gen_ptr, const char * var_name, void ** var_p_ptr)
	{
	pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1_state		*prs_ptr;

	FIN_MT (_op_pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1_svar (gen_ptr, var_name, var_p_ptr))
	...
	prs_ptr = (pri_proc_enhanced_hrc_uniformly_assign_no_DDC_in_grade1_state *)gen_ptr;
	...
	if (strcmp ("sink_neighbor_list" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->sink_neighbor_list);
		FOUT
		}
	...
	if (strcmp ("sent_pkts_num" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->sent_pkts_num);
		FOUT
		}
